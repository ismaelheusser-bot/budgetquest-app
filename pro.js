...
const link=document.createElement('link');link.rel='stylesheet';link.href='pro.css?v=11';document.head.appendChild(link);
function inject(){
 ...
 if(!q('#wealth')){
  ...
  q('.nav').insertAdjacentHTML('beforeend','<button data-target="wealth">Vermögen</button>');
  q('.nav').style.gridTemplateColumns='repeat(7,1fr)';
  bindNav();
...
inject();bindNav();const old=window.render;window.render=function(){old();renderPro()};renderPro();
})();